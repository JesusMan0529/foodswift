...
项目核心数据规模为 **11 张业务表、69 个后端 HTTP 接口、11 个小程序页面**，同时提供 WebSocket 实时通知、Redis 缓存、百度地图配送范围校验以及 Excel 运营数据导出。

## 业务角色与功能全景

| 角色 | 使用入口 | 核心目标 | 主要能力 |
| --- | --- | --- | --- |
| 消费者 | 微信小程序 | 快速完成选购和履约 | 微信登录、查看营业状态、浏览菜品/套餐、选择口味、维护购物车、管理地址、填写备注、选择配送时间与餐具、支付、取消、催单、查看历史订单、再来一单 |
...
| 系统服务 | Java 后端 | 保证业务自动运行 | JWT 鉴权、公共字段自动填充、缓存同步、超时订单处理、派送订单完成、WebSocket 通知、异常统一处理 |
