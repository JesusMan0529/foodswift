...
import com.fs.exception.AddressBookBusinessException;
import com.fs.exception.OrderBusinessException;
import com.fs.exception.ShoppingCartBusinessException;
...
public class OrderServiceImpl implements OrderService {
    ...
    public void payment(OrdersPaymentDTO ordersPaymentDTO) {
        ...
        // 只允许当前用户支付自己的订单
        if (ordersDB == null || !Objects.equals(ordersDB.getUserId(), userId)) {
            throw new OrderBusinessException(MessageConstant.ORDER_NOT_FOUND);
        }

        // 模拟支付只能处理“待付款 + 未支付”的订单
        if (!Objects.equals(ordersDB.getStatus(), Orders.PENDING_PAYMENT)
                || !Objects.equals(ordersDB.getPayStatus(), Orders.UN_PAID)) {
    ...
    private void checkOutOfRange(String address) {
        //委托给地图距离组件校验
        mapDistanceComponent.checkOutOfRange(address);
    }
}
